...
def plot_errs_vs_dt_geni(TG, f, width=70, alphas = (0, 0.05, 0.07, 0.1, 0.15, 0.25, 0.5)) :
    fs = []
    labels = []
    for a in alphas:
        fs.append( (lambda a=a: lambda Q, U : (1-a)*Q + a*f(Q,U) )() )
        labels.append("$\\alpha=%.2f$" % a)
        
    plot_errs_vs_dt(TG, fs, labels, width)

def plot_errs_vs_dt(TG, fs, labels=None, width=40):

	def chop(*ars, ends=len(TG.Gs)//2 - width//2 ):
	...
	plot_errors(chop(*Es), chop(T)[0], cbar_labels=labels, max_bars = 100)
